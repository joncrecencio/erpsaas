...
class OrderController extends Controller
{
    ...
    public function completa($id)
    {
        ...
        // Alteração para carregar apenas o relatório da ordem de serviço atual
        $relatorio = RelatorioOs::where('ordem_servico_id', $id)->get();
    ...
    {
        // Removendo produtos relacionados
        foreach ($item->produtos as $p) {
            $p->delete();
        }
    
        // Removendo serviços relacionados
        foreach ($item->servicos as $s) {
            $s->delete();
        }
    
        // Removendo relatórios relacionados
        foreach ($item->relatorios as $r) {
            $r->delete();
        }
    
        // Removendo funcionários relacionados
        foreach ($item->funcionarios as $f) {
            $f->delete();
        }
    }
